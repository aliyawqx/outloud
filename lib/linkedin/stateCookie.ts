...
// /callback reads. Mirrors lib/threads/stateCookie.ts. LinkedIn's code flow has
// no PKCE verifier (the client secret authenticates the exchange, server-side),
// so only `state` (and an optional returnTo) travel in the cookie.

export const LINKEDIN_OAUTH_COOKIE = 'linkedin_oauth_tx'
export const LINKEDIN_OAUTH_MAX_AGE_S = 600 // 10 minutes
...
export async function sealOAuthTx(tx: OAuthTx): Promise<string> {
  return new SignJWT({ state: tx.state, ...(tx.returnTo ? { returnTo: tx.returnTo } : {}) })
    ...
    .setIssuedAt()
    .setExpirationTime(`${LINKEDIN_OAUTH_MAX_AGE_S}s`)
    .sign(secret())
}
